...
A `.env` file is required to provide database configurations to Docker. Create a `.env` file in the root folder (`todo_list/.env`) and paste the following:
...
_Note: The first build might take a few minutes as Docker needs to download the dependencies._
...
_(Add_ `-v` _to remove the database volume if you want to wipe the data)._
